...
void init(GLFWwindow* window) {
	...
	renderingProgramGomb = createShaderProgram();
...
void display() {
	...
	//kört elkezdtem de vgül nem lett megcsinálva
	/*float radiusKor = 3.0f;
	lightPos.x = radiusKor * cos(currentTime);
	lightPos.y = radiusKor * sin(currentTime);
	lightPos.z = 1.5f;

	glm::vec3 lightColor = glm::vec3(1.0f, 0.0f, 0.0f);

	glUseProgram(renderingProgramGomb);
	...
	if ((keyboard[GLFW_KEY_W]) || (keyboard[GLFW_KEY_UP]))
		height = glm::clamp(height + cameraSpeed, -1000.0f, 1000.0f);
	...
	// light follow the user viewing the scene, behaves as a headlamp //fix fényforrás
	//lightPos = cameraPos;
